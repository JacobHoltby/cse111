...
def display_periodic_table(list):
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for item in (symbol_quantity_list):
        element = item[SYMBOL_INDEX]
        atomic_mass = periodic_table_dict[element][QUANTITY_INDEX]
...
def main():
    ...
    # display_periodic_table(list)
    # print(f'{periodic_table_list}')

    formula = input('Enter the molecular formula of the sample: ')
    sample_mass = float(input('Enter the mass in grams of the sample: '))
    compound_list = parse_formula(formula, list)
    print(compound_list)
    molar_mass = compute_molar_mass(compound_list, list)
    ...
    if formula in known_molecules_dict:
        print(f'This molecule is {known_molecules_dict[formula]}.')
